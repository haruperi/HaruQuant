...
1. **Use the Engine Path:** `engine_type` is kept for compatibility, but optimization execution now routes through the current `apps.trading.Engine` path.
...
2. Implement function signature:
   ...
   def my_new_method(
       ...
       scoring_func,
       initial_balance=10000.0,
       engine_type="vectorized",  # accepted for compatibility; runs through apps.trading.Engine
       verbose=True,
