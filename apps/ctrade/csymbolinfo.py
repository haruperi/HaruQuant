"""
CSymbolInfo class (MT5-specific).

This module mirrors the MQL5 Standard Library CSymbolInfo interface and
...
class CSymbolInfo:
    ...
    This class is based on the MQL5 Standard Library CSymbolInfo API.
    """

    def __init__(self, symbol: str = "") -> None:
        """
        Create a CSymbolInfo instance.
    ...
    def _info_integer(self, prop: Optional[int]) -> Optional[int]:
        if prop is None:
            return None
        if hasattr(mt5, "symbol_info_integer"):
            value = mt5.symbol_info_integer(self._symbol, prop)
    ...
    def _info_double(self, prop: Optional[int]) -> Optional[float]:
        if prop is None:
            return None
        if hasattr(mt5, "symbol_info_double"):
            value = mt5.symbol_info_double(self._symbol, prop)
    ...
    def _info_string(self, prop: Optional[int]) -> Optional[str]:
        if prop is None:
            return None
        if hasattr(mt5, "symbol_info_string"):
            value = mt5.symbol_info_string(self._symbol, prop)
    ...
    # ---------------------------------------------------------------------
    # Class methods by groups (per MQL5 CSymbolInfo documentation)
    # ---------------------------------------------------------------------
    # Controlling
    ...
    def Select(self, select: Optional[bool] = None) -> bool:
        ...
        if select is None:
            return bool(self._cached(False, "select", "visible"))
        return bool(mt5.symbol_select(self._symbol, bool(select)))
    ...
    def InfoInteger(self, prop: Any) -> Optional[int]:
        ...
        if isinstance(prop, int) and hasattr(mt5, "symbol_info_integer"):
            value = mt5.symbol_info_integer(self._symbol, prop)
            return int(value) if value is not None else None
    ...
    def InfoDouble(self, prop: Any) -> Optional[float]:
        ...
        if isinstance(prop, int) and hasattr(mt5, "symbol_info_double"):
            value = mt5.symbol_info_double(self._symbol, prop)
            return float(value) if value is not None else None
        if isinstance(prop, str):
            value = self._cached(None, prop)
    ...
    def InfoString(self, prop: Any) -> Optional[str]:
        ...
        if isinstance(prop, int) and hasattr(mt5, "symbol_info_string"):
            value = mt5.symbol_info_string(self._symbol, prop)
            return str(value) if value is not None else None
        if isinstance(prop, str):
            value = self._cached(None, prop)
