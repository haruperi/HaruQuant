"""
CAccountInfo class (MT5-specific).

This module mirrors the MQL5 Standard Library CAccountInfo interface and
...
class CAccountInfo:
    ...
    This class is based on the MQL5 Standard Library CAccountInfo API.
    """
    ...
    def _info_integer(self, prop: Optional[int]) -> Optional[int]:
        if prop is None:
            return None
        if hasattr(mt5, "account_info_integer"):
            value = mt5.account_info_integer(prop)
    ...
    def _info_double(self, prop: Optional[int]) -> Optional[float]:
        if prop is None:
            return None
        if hasattr(mt5, "account_info_double"):
            value = mt5.account_info_double(prop)
    ...
    def _info_string(self, prop: Optional[int]) -> Optional[str]:
        if prop is None:
            return None
        if hasattr(mt5, "account_info_string"):
            value = mt5.account_info_string(prop)
    ...
    # ---------------------------------------------------------------------
    # Class methods by groups (per MQL5 CAccountInfo documentation)
    # ---------------------------------------------------------------------
    ...
    def InfoInteger(self, prop: Any) -> Optional[int]:
        ...
        if isinstance(prop, int) and hasattr(mt5, "account_info_integer"):
            value = mt5.account_info_integer(prop)
            return int(value) if value is not None else None
    ...
    def InfoDouble(self, prop: Any) -> Optional[float]:
        ...
        if isinstance(prop, int) and hasattr(mt5, "account_info_double"):
            value = mt5.account_info_double(prop)
            return float(value) if value is not None else None
        if isinstance(prop, str):
            value = self._cached(None, prop)
    ...
    def InfoString(self, prop: Any) -> Optional[str]:
        ...
        if isinstance(prop, int) and hasattr(mt5, "account_info_string"):
            value = mt5.account_info_string(prop)
            return str(value) if value is not None else None
        if isinstance(prop, str):
            value = self._cached(None, prop)
