"""
DealInfo class (MT5-specific).

This module mirrors the MQL5 Standard Library DealInfo interface and
...
class DealInfo:
    """
    Class for handling deal properties.

    This class is based on the MQL5 Standard Library DealInfo API.
    """

    def __init__(self, ticket: int = 0, api: Optional[Any] = None) -> None:
        """Create a DealInfo instance."""
    ...
    def _info_integer(self, prop: Optional[int]) -> Optional[int]:
        if prop is None:
            return None
        if hasattr(self._api, "history_deal_get_integer"):
            value = self._api.history_deal_get_integer(prop)
    ...
    def _info_double(self, prop: Optional[int]) -> Optional[float]:
        if prop is None:
            return None
        if hasattr(self._api, "history_deal_get_double"):
            value = self._api.history_deal_get_double(prop)
    ...
    def _info_string(self, prop: Optional[int]) -> Optional[str]:
        if prop is None:
            return None
        if hasattr(self._api, "history_deal_get_string"):
            value = self._api.history_deal_get_string(prop)
    ...
    # ---------------------------------------------------------------------
    # Deal Selection and History
    # ---------------------------------------------------------------------
    def Select(self, ticket: int) -> bool:
        """Select the deal by ticket."""
        deals = self._api.history_deals_get(ticket=ticket)
        if not deals:
            return False
    ...
    def HistorySelect(self, date_from: datetime, date_to: datetime) -> bool:
        ...
        deals = self._api.history_deals_get(date_from, date_to)
        if deals is None:
            self._history = []
            return False
        self._history = deals
    ...
    def TotalDeals(self) -> int:
        """Get the total number of deals in history selection."""
    ...
    def SelectByIndex(self, index: int) -> bool:
        ...
        if index < 0 or index >= len(self._history):
            return False
        return self._set_deal(self._history[index])
    ...
    def InfoInteger(self, prop: Any) -> Optional[int]:
        ...
        if isinstance(prop, int) and hasattr(self._api, "history_deal_get_integer"):
            value = self._api.history_deal_get_integer(prop)
            return int(value) if value is not None else None
    ...
    def InfoDouble(self, prop: Any) -> Optional[float]:
        ...
        if isinstance(prop, int) and hasattr(self._api, "history_deal_get_double"):
            value = self._api.history_deal_get_double(prop)
            return float(value) if value is not None else None
        if isinstance(prop, str):
            value = self._cached(None, prop)
    ...
    def InfoString(self, prop: Any) -> Optional[str]:
        ...
        if isinstance(prop, int) and hasattr(self._api, "history_deal_get_string"):
            value = self._api.history_deal_get_string(prop)
            return str(value) if value is not None else None
        if isinstance(prop, str):
            value = self._cached(None, prop)
