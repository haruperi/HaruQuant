"""
OrderInfo class (MT5-specific).

This module mirrors the MQL5 Standard Library OrderInfo interface and
...
class OrderInfo:
    """
    Class for handling order properties (active orders).

    This class is based on the MQL5 Standard Library OrderInfo API.
    """
    ...
    def _info_integer(self, prop: Optional[int]) -> Optional[int]:
        if prop is None:
            return None
        if hasattr(self._api, "order_get_integer"):
            value = self._api.order_get_integer(prop)
    ...
    def _info_double(self, prop: Optional[int]) -> Optional[float]:
        if prop is None:
            return None
        if hasattr(self._api, "order_get_double"):
            value = self._api.order_get_double(prop)
    ...
    def _info_string(self, prop: Optional[int]) -> Optional[str]:
        if prop is None:
            return None
        if hasattr(self._api, "order_get_string"):
            value = self._api.order_get_string(prop)
    ...
    # ---------------------------------------------------------------------
    # Order Selection and Access
    # ---------------------------------------------------------------------
    def Select(self, ticket: int) -> bool:
        """Select order by ticket."""
        if hasattr(self._api, "order_select"):
            ok = self._api.order_select(ticket)
            if ok:
                return self._set_order(self._api.order_get(ticket))
            return False
        return False
    ...
    def InfoInteger(self, prop: Any) -> Optional[int]:
        ...
        if isinstance(prop, int) and hasattr(self._api, "order_get_integer"):
            value = self._api.order_get_integer(prop)
            return int(value) if value is not None else None
    ...
    def InfoDouble(self, prop: Any) -> Optional[float]:
        ...
        if isinstance(prop, int) and hasattr(self._api, "order_get_double"):
            value = self._api.order_get_double(prop)
            return float(value) if value is not None else None
        if isinstance(prop, str):
            value = self._cached(None, prop)
    ...
    def InfoString(self, prop: Any) -> Optional[str]:
        ...
        if isinstance(prop, int) and hasattr(self._api, "order_get_string"):
            value = self._api.order_get_string(prop)
            return str(value) if value is not None else None
        if isinstance(prop, str):
            value = self._cached(None, prop)
