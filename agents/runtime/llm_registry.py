...
def get_provider(model: Optional[str] = None, provider: Optional[str] = None) -> Type[LLMRuntime]:
    ...
    if model:
        ...
        if "gemini" in model_lower:
            # Try litellm first (handles gemini via API key), then google-adk
            for p in ["litellm", "google-adk", "openai"]:
                cls = _PROVIDERS.get(p)
                if cls:
                    return cls
        if any(x in model_lower for x in ("gpt-", "o1", "o3", "claude", "gpt-5")):
            for p in ["openai", "litellm"]:
                cls = _PROVIDERS.get(p)
                if cls:
                    return cls
        # Ollama models: llama, qwen, gemma, phi, mistral, etc.
...
def _register_builtin_providers() -> None:
    ...
    try:
        ...
        from agents.runtime.litellm_runtime import LiteLLMRuntime
    ...
    try:
        ...
        from agents.runtime.openai_runtime import OpenAIRuntime
    ...
    try:
        ...
        from agents.runtime.google_adk_runtime import GoogleADKRuntime
