...
def test_service_returns_agent_response():
    response = asyncio.run(StrategyCostExecutionAgentService().run(_request(), AgentContext(session_id="ctx-test")))
    assert response.agent_name == "strategy_cost_execution_agent"
