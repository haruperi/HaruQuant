...
def test_service_returns_agent_response():
    response = asyncio.run(StrategySpecValidatorAgentService().run(_request(), AgentContext(session_id="ctx-test")))
    assert response.agent_name == "strategy_spec_validator_agent"
