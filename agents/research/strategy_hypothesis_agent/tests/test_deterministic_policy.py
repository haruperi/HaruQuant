...
def test_normal_policy_allows_research_actions():
    decision = make_final_decision([EvidenceItem(source="candidate_strategy_idea", description="ok", value={"data_quality_score": 0.9, "sample_size": 250})], None)
    assert decision.status == AgentStatus.SUCCESS
    assert decision.allowed_actions


def test_extreme_volatility_blocks_execution():
    decision = make_final_decision([EvidenceItem(source="candidate_strategy_idea", description="risk", value={"volatility_state": "extreme"})], None)
    assert decision.risk_level == RiskLevel.CRITICAL
    assert "trade_execution" in decision.blocked_actions


def test_llm_cannot_override_rejected_validation():
    decision = make_final_decision([EvidenceItem(source="candidate_strategy_idea", description="reject", value={"validation_status": "rejected"})], None)
