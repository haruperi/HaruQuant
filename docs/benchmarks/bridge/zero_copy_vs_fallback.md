...
Date: 2026-02-17
Environment: local Windows Release build (`build/bridge/Release/hqt_engine.pyd`)
...
n = 200_000
print("sum_buffer_zero_copy_us", timeit.timeit("hqt_engine.sum_buffer_zero_copy(arr)", number=n, globals=globals()) / n * 1e6)
print("sum_auto_zero_copy_us", timeit.timeit("hqt_engine.sum_auto(arr)", number=n, globals=globals()) / n * 1e6)
print("sum_auto_copy_fallback_us", timeit.timeit("hqt_engine.sum_auto(listv)", number=n, globals=globals()) / n * 1e6)
```
