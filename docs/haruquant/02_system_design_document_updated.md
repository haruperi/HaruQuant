...
### 21.1 Class Diagram (Core Hybrid Architecture)
...
class SignalBuilder {
  +build_signals(data, params) SignalArrays
  +validate_alignment()
}

class DataLoader {
  +load_parquet(symbol, timeframe)
  +normalize_arrays(df) MarketArrays
  +validate_schema()
}

class HqtEngineBinding {
  +run_bars_signals(market, signals, config) RunResult
  +run_event_mode(stream, config) RunResult
  +run_batch(market_pack, signal_pack, config_pack, n_threads) BatchResult
  +version() str
}

class SignalEngine {
  +run(market, signals, config) RunResult
  -process_bar(i)
  -apply_exit_rules(i)
  -apply_entry_rules(i)
  -mark_equity(i)
...
class EventEngine {
  +run(event_stream, config) RunResult
  -dispatch_event(e)
...
class BatchRunner {
  +run_batch(pack, n_threads) BatchResult
  -schedule_jobs()
  -reduce_top_k()
}

class ExecutionModel {
  +fill_market(order, bar, config) Fill
  +compute_slippage(order, bar)
  +compute_commission(order)
  +compute_swap(position, dt)
}

class Portfolio {
  +open_position(fill)
  +close_position(order)
  +update_mark_to_market(bar)
  +equity() double
  +free_margin() double
}

class RiskGovernor {
  +can_open(order, account_state) bool
  +validate_limits(account_state) bool
  +trip_circuit_breaker(reason)
}

class MetricsEngine {
  +compute_summary(trades, equity) Metrics
  +max_drawdown(equity) double
  +profit_factor(trades) double
  +expectancy(trades) double
}
...
class ResultsRepository {
  ...
  +save_trades(run_id, trades)
  +save_artifact(run_id, path)
}

class NotificationService {
  +notify_critical(event)
  +notify_status(event)
}

PythonOrchestrator --> DataLoader
PythonOrchestrator --> SignalBuilder
PythonOrchestrator --> HqtEngineBinding
PythonOrchestrator --> ResultsRepository
PythonOrchestrator --> NotificationService

HqtEngineBinding --> SignalEngine
HqtEngineBinding --> EventEngine
HqtEngineBinding --> BatchRunner

SignalEngine --> ExecutionModel
SignalEngine --> Portfolio
SignalEngine --> RiskGovernor
SignalEngine --> MetricsEngine

EventEngine --> ExecutionModel
EventEngine --> Portfolio
EventEngine --> RiskGovernor
EventEngine --> MetricsEngine

BatchRunner --> SignalEngine
BatchRunner --> MetricsEngine

PythonOrchestrator --> MT5Gateway : live/paper
MT5Gateway --> RiskGovernor
```

### 21.2 Sequence Diagram — Signal-Driven Backtest

```mermaid
sequenceDiagram
autonumber
actor User
participant PY as PythonOrchestrator
participant DL as DataLoader
participant SB as SignalBuilder
participant NB as HqtEngineBinding
participant SE as SignalEngine
participant EX as ExecutionModel
participant PF as Portfolio
participant ME as MetricsEngine
participant RR as ResultsRepository

User->>PY: run_backtest(config, symbol, timeframe, params)
PY->>DL: load_parquet(symbol, timeframe)
DL-->>PY: market_df
PY->>DL: normalize_arrays(market_df)
DL-->>PY: MarketArrays
PY->>SB: build_signals(MarketArrays, params)
SB-->>PY: SignalArrays
PY->>NB: run_bars_signals(MarketArrays, SignalArrays, config)

NB->>SE: run(market, signals, config)
loop for each bar i
  SE->>SE: apply exit/entry policy (deterministic order)
  SE->>EX: fill_market()/fees/slippage as needed
  EX-->>SE: Fill/Costs
  SE->>PF: update position/account/equity
  PF-->>SE: account_state
end
SE->>ME: compute_summary(trades, equity)
ME-->>SE: metrics
SE-->>NB: RunResult(trades, equity, metrics)
NB-->>PY: RunResult
PY->>RR: save_run + save_metrics + save_trades + save_artifacts
RR-->>PY: persisted IDs/paths
PY-->>User: completed(run_id, headline metrics)
```

### 21.3 Sequence Diagram — Batch Optimization (Parallel)
...
User->>PY: run_batch(base_config, param_grid)
PY->>DL: load/normalize market arrays
DL-->>PY: MarketArrays
loop each param set
  PY->>SB: build_signals(MarketArrays, params_k)
  SB-->>PY: SignalArrays_k
end
PY->>NB: run_batch(market_pack, signal_pack, config_pack, n_threads)
NB->>BR: run_batch(pack, n_threads)

par worker threads
  BR->>SE: run(market, signals_k, config_k)
  SE-->>BR: summary_k + optional artifacts_k
end

BR->>BR: rank results / top-K reduction
BR-->>NB: BatchResult(leaderboard, topK)
NB-->>PY: BatchResult
PY->>RR: persist leaderboard + topK artifacts
RR-->>PY: saved references
PY-->>User: optimization completed (best configs)
```

### 21.4 Sequence Diagram — Live Trading Flow (MT5 Bridge)

```mermaid
sequenceDiagram
autonumber
actor Operator
participant PY as Python Live Controller
participant RG as RiskGovernor
participant MG as MT5Gateway
participant EA as MQL5 EA (Terminal)
participant RR as ResultsRepository
participant NS as NotificationService

Operator->>PY: start_live(strategy, config)
PY->>MG: connect()
MG-->>PY: connected
PY->>MG: sync_state()
MG->>EA: state request
EA-->>MG: positions/orders/account
MG-->>PY: reconciled snapshot

loop on new tick/signal
  PY->>RG: can_open?/validate_limits
  alt allowed
    PY->>MG: send_order(request)
    MG->>EA: execute order
    EA-->>MG: execution report
    MG-->>PY: ack/fill
    PY->>RR: persist trade/update state
  else blocked
    PY->>NS: notify_critical("risk gate blocked order")
  end
end

alt disconnect/failure
  MG-->>PY: connection_lost
  PY->>NS: notify_critical("bridge disconnected")
  PY->>PY: safe-halt / retry backoff
end
```
