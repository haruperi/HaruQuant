...
from services.strategy.evidence.audit import ReplayBundleAssembler, build_audit_export_package
...
def _seed_workflow(database_path: Path) -> None:
...
def test_phase4_scenario_supervised_paper_execution_creates_full_replay_trail(tmp_path) -> None:
    migrations_dir = default_migrations_dir()
    database_path = tmp_path / "agentic.db"

    apply_pending_migrations(database_path, migrations_dir)
    _seed_workflow(database_path)
    ...
    send_result = ExecutionSendService(_ScenarioBrokerGateway()).send(intent)
    ...
    audit_repository.create_evidence_bundle(
        ...
        summary="Control-plane scenario evidence",
