...
def test_duplicate_receipt_chaos_scenario_keeps_latest_receipt_addressable(tmp_path) -> None:
    ...
    with repository._connect() as connection:  # noqa: SLF001
        connection.execute(
            ...
            ("wf_001", "trade_review", "paper", "MODE-003", "CREATED", "duplicate receipt", "{}", "user", "operator_001", "{}", "[]"),
        )
        connection.execute(
            """
        ...
        connection.execute(
            "INSERT INTO core_execution_intents (execution_intent_id, workflow_id, proposal_id, risk_decision_id, action_type, symbol, side, order_type, size_json, price_params_json, sl_tp_params_json, idempotency_key, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            ("exec_001", "wf_001", "prop_001", "risk_001", "submit_order", "EURUSD", "buy", "market", "{}", "{}", "{}", "idem_001", "SENT"),
        )

    first = repository.add_receipt(
        receipt_id="rcpt_001",
    ...
    latest = repository.get_latest_receipt_for_intent("exec_001")

    assert first.execution_intent_id == "exec_001"
    assert second.execution_intent_id == "exec_001"
    assert latest is not None
    assert latest.receipt_id == "rcpt_002"
    assert latest.broker_order_id == "100"
