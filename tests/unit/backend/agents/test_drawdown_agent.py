    ...
    def run(self, *, request, context):  # noqa: ANN001
        return AgentExecutionResult(
            output_payload={
                ...
                "originator": {"type": "agent", "id": "drawdown_agent"},
                "environment": "paper",
                ...
                "payload": {
                    ...
                    "observation_type": "drawdown_summary",
                    "severity": "warning",
                    "source": "drawdown_agent",
                    "payload_ref_or_inline": {"drawdown_pct": 0.08, "state": "elevated"},
...
def test_drawdown_agent_wrapper_validates_drawdown_output_schema() -> None:
    wrapper = DrawdownAgentWrapper(
    ...
    result = wrapper.execute(
        runtime_agent=FakeDrawdownRuntime(),
        request=ADKRunRequest(
            workflow_id="wf_001",
            correlation_id="corr_001",
            agent_name="drawdown_agent",
            input_payload={"goal": "Summarize drawdown state"},
        ),
    )

    assert "drawdown state" in DRAWDOWN_AGENT_INSTRUCTION
