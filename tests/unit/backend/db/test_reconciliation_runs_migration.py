...
def test_reconciliation_runs_migration_supports_conflict_flag(tmp_path) -> None:
    ...
    try:
        ...
        connection.execute(
            "INSERT INTO core_reconciliation_runs (execution_intent_id, run_reason, result_state, broker_truth_json, local_truth_json, conflict_flag, incident_id, completed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            ("exec_001", "restart_recovery", "CONFLICT", '{"broker_status":"filled"}', '{"local_status":"pending"}', 1, "inc_001", "2026-04-08T12:40:00Z"),
        )
        connection.commit()

        row = connection.execute(
            "SELECT execution_intent_id, result_state, conflict_flag FROM core_reconciliation_runs"
        ).fetchone()
    finally:
        connection.close()

    assert row == ("exec_001", "CONFLICT", 1)
