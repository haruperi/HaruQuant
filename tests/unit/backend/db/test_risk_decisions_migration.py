...
def test_risk_decisions_migration_enforces_decision_enum_and_unique_token(tmp_path) -> None:
    ...
    try:
        ...
        row = connection.execute(
            "SELECT risk_decision_id, decision, approval_token FROM risk_risk_decisions"
        ).fetchone()

        duplicate_token_failed = False
        try:
            connection.execute(
                "INSERT INTO risk_risk_decisions (risk_decision_id, risk_request_id, proposal_id, workflow_id, decision, rationale_text, risk_metrics_snapshot_json, freshness_expiry, policy_version_id, formula_version, provenance_bundle_id, approval_token, freshness_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                ("risk_dec_002", "risk_req_001", "prop_001", "wf_001", "REJECT", "Duplicate token", '{"var":0.03}', "2026-04-08T12:45:00Z", "risk_policy_1", "formula_1", None, "approval_001", "fresh"),
            )
            connection.commit()
        except sqlite3.IntegrityError:
            duplicate_token_failed = True
            connection.rollback()
    finally:
        connection.close()

    assert row == ("risk_dec_001", "APPROVE", "approval_001")
    assert duplicate_token_failed is True
