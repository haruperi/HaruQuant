...
def test_risk_assessment_requests_migration_supports_insert_and_fks(tmp_path) -> None:
    ...
    try:
        ...
        connection.execute(
            ...
            INSERT INTO risk_risk_assessment_requests (
                risk_request_id, workflow_id, proposal_id, action_type,
                account_snapshot_ref, portfolio_snapshot_ref, market_snapshot_ref,
                requested_freshness_json, strategy_lifecycle_state,
                active_policy_bundle_json, compliance_profile_id,
                current_kill_switch_state
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                "risk_req_001", "wf_001", "prop_001", "open_position",
                "acct_snap_001", "port_snap_001", "mkt_snap_001",
                '{"market":"fresh"}', "PAPER_VERIFIED",
                '{"policy_version":"risk_policy_1"}', None, "DISARMED"
            ),
        )
        connection.commit()

        row = connection.execute(
            "SELECT risk_request_id, proposal_id, action_type FROM risk_risk_assessment_requests"
        ).fetchone()
        foreign_keys = connection.execute(
            "PRAGMA foreign_key_list('risk_risk_assessment_requests')"
        ).fetchall()
    finally:
        connection.close()

    assert row == ("risk_req_001", "prop_001", "open_position")
    assert any(fk[2] == "core_workflows" for fk in foreign_keys)
    assert any(fk[2] == "core_trade_proposals" for fk in foreign_keys)
