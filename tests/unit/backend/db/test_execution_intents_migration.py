...
def test_execution_intents_migration_enforces_idempotency_uniqueness(tmp_path) -> None:
    ...
    try:
        ...
        connection.execute(
            "INSERT INTO core_execution_intents (execution_intent_id, workflow_id, proposal_id, risk_decision_id, action_type, symbol, side, order_type, size_json, price_params_json, sl_tp_params_json, idempotency_key, client_order_id, status, expiry_at, pre_send_validation_snapshot_ref) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            ("exec_001", "wf_001", "prop_001", "risk_dec_001", "open_position", "EURUSD", "buy", "limit", '{"units":1000}', '{"limit_price":1.0832}', '{"stop_loss":1.0800}', "idem_001", "client_001", "PENDING", "2026-04-08T12:45:00Z", "presend_001"),
        )
        connection.commit()

        row = connection.execute(
            "SELECT execution_intent_id, idempotency_key FROM core_execution_intents"
        ).fetchone()
        ...
        try:
            connection.execute(
                "INSERT INTO core_execution_intents (execution_intent_id, workflow_id, proposal_id, risk_decision_id, action_type, symbol, side, order_type, size_json, price_params_json, sl_tp_params_json, idempotency_key, client_order_id, status, expiry_at, pre_send_validation_snapshot_ref) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                ("exec_002", "wf_001", "prop_001", "risk_dec_001", "open_position", "EURUSD", "buy", "limit", '{"units":1000}', '{"limit_price":1.0833}', '{"stop_loss":1.0801}', "idem_001", "client_002", "PENDING", "2026-04-08T12:46:00Z", "presend_002"),
            )
            connection.commit()
