...
from backend_retiring.agents.chat.ai_chat.conversation_planner import ConversationPlanner, StructuredChatPlan
...
def test_gateway_default_planner_uses_live_llm_assist_when_needed(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("backend_retiring.agents.chat.ai_chat.conversation_planner.create_llm_runtime", fake_create_llm_runtime)

    service = ConversationService(AiChatRepository(database_path))
    thread = service.create_thread(user_id=1, current_route="/strategies", current_page_type="generic")
    gateway = __import__("backend_retiring.agents.chat.ai_chat", fromlist=["AIGatewayService"]).AIGatewayService(
        conversation_service=service,
        context_assembler=PageContextAssembler(db_manager=db),
    )

    metadata, chunks, _message_id = gateway.stream_response(
        __import__("backend_retiring.agents.chat.ai_chat", fromlist=["ChatStreamRequest"]).ChatStreamRequest(
            user_id=1,
            thread_id=thread.thread_id,
            prompt="I want something mean-reverting but not too aggressive",
        )
