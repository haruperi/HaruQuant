...
def message_for(code: int) -> t.Optional[str]:
    return _MT5_MESSAGES.get(code)


def error_from_retcode(code: int) -> ErrorInfo:
    if code == 0:
        return ErrorInfo(0, "OK", "The operation completed successfully")
    elif code == 10004:
        return ErrorInfo(10004, "TRADE_RETCODE_REQUOTE", "Requote", "trade", True)
    elif code == 10006:
        return ErrorInfo(10006, "TRADE_RETCODE_REJECT", "Request rejected", "trade", True)
    elif code == 10007:
        return ErrorInfo(10007, "TRADE_RETCODE_CANCEL", "Request canceled by trader")
    elif code == 10008:
        return ErrorInfo(10008, "TRADE_RETCODE_PLACED", "Order placed")
    elif code == 10009:
        return ErrorInfo(10009, "TRADE_RETCODE_DONE", "Request completed")
    elif code == 10010:
        return ErrorInfo(10010, "TRADE_RETCODE_DONE_PARTIAL", "Only part of the request was completed")
    elif code == 10011:
        return ErrorInfo(10011, "TRADE_RETCODE_ERROR", "Request processing error", "trade", True)
    elif code == 10012:
        return ErrorInfo(10012, "TRADE_RETCODE_TIMEOUT", "Request canceled by timeout", "trade", True)
    elif code == 10013:
        return ErrorInfo(10013, "TRADE_RETCODE_INVALID", "Invalid request")
    elif code == 10014:
        return ErrorInfo(10014, "TRADE_RETCODE_INVALID_VOLUME", "Invalid volume")
    elif code == 10015:
        return ErrorInfo(10015, "TRADE_RETCODE_INVALID_PRICE", "Invalid price")
    elif code == 10016:
        return ErrorInfo(10016, "TRADE_RETCODE_INVALID_STOPS", "Invalid stops")
    elif code == 10017:
        return ErrorInfo(10017, "TRADE_RETCODE_TRADE_DISABLED", "Trade is disabled")
    elif code == 10018:
        return ErrorInfo(10018, "TRADE_RETCODE_MARKET_CLOSED", "Market is closed", "trade", True)
    elif code == 10019:
        return ErrorInfo(10019, "TRADE_RETCODE_NO_MONEY", "There is not enough money to complete the request")
    elif code == 10020:
        return ErrorInfo(10020, "TRADE_RETCODE_PRICE_CHANGED", "Prices changed", "trade", True)
    elif code == 10021:
        return ErrorInfo(10021, "TRADE_RETCODE_PRICE_OFF", "There are no quotes to process the request", "trade", True)
    elif code == 10022:
        return ErrorInfo(10022, "TRADE_RETCODE_INVALID_EXPIRATION", "Invalid expiration")
    elif code == 10023:
        return ErrorInfo(10023, "TRADE_RETCODE_ORDER_CHANGED", "Order state changed", "trade", True)
    elif code == 10024:
        return ErrorInfo(10024, "TRADE_RETCODE_TOO_MANY_REQUESTS", "Too frequent requests", "trade", True)
    elif code == 10025:
        return ErrorInfo(10025, "TRADE_RETCODE_NO_CHANGES", "No changes in request")
    elif code == 10026:
        return ErrorInfo(10026, "TRADE_RETCODE_SERVER_DISABLES_AT", "Autotrading disabled by server")
    elif code == 10027:
        return ErrorInfo(10027, "TRADE_RETCODE_CLIENT_DISABLES_AT", "Autotrading disabled by client")
    elif code == 10028:
        return ErrorInfo(10028, "TRADE_RETCODE_LOCKED", "Request locked for processing", "trade", True)
    elif code == 10029:
        return ErrorInfo(10029, "TRADE_RETCODE_FROZEN", "Order or position frozen")
    elif code == 10030:
        return ErrorInfo(10030, "TRADE_RETCODE_INVALID_FILL", "Invalid order filling type")
    elif code == 10031:
        return ErrorInfo(10031, "TRADE_RETCODE_CONNECTION", "No connection with the trade server", "trade", True)
    elif code == 10032:
        return ErrorInfo(10032, "TRADE_RETCODE_ONLY_REAL", "Only long positions allowed")
    elif code == 10033:
        return ErrorInfo(10033, "TRADE_RETCODE_LIMIT_ORDERS", "Only short positions allowed")
    elif code == 10034:
        return ErrorInfo(10034, "TRADE_RETCODE_LIMIT_VOLUME", "Only closing positions allowed")
    elif code == 10035:
        return ErrorInfo(10035, "TRADE_RETCODE_INVALID_ORDER", "Position closed")
    elif code == 10036:
        return ErrorInfo(10036, "TRADE_RETCODE_POSITION_CLOSED", "Invalid close volume")

    message = message_for(code)
    if message:
        return ErrorInfo(code, "MT5_RUNTIME_ERROR", message, "mt5")

    if code >= 65536:
        return ErrorInfo(code, "USER_ERROR", f"User error {code - 65536}", "mt5", False)

    return ErrorInfo(code, "UNKNOWN", "Unknown error", "trade", False)
...
def is_success_retcode(code: int) -> bool:
    return code in (0, 10008, 10009, 10010)

def raise_exception_for_retcode(code: int, msg: str = "") -> None:
    error_info = error_from_retcode(code)
    full_msg = f"{error_info.name} [{code}]: {error_info.message}"
    if msg:
        full_msg = f"{msg} | {full_msg}"
    
    # Normally we might map to specific Exception types based on ErrorInfo.domain or name
    raise RuntimeError(full_msg)

def raise_exception_for_category(category: str, msg: str) -> None:
    raise ValueError(f"{category.upper()} Error: {msg}")
