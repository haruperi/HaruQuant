...
def get_mt5_credentials(user_id: int = 1, mt5_login: Optional[int] = None) -> dict[str, Any] | None:
    """Load MT5 credentials from the application database."""
    from data.database.sqlite.users import UserManager

    user_manager = UserManager()
    if mt5_login:
        return user_manager.get_mt5_credentials_by_login(user_id, mt5_login)
    return user_manager.get_mt5_credentials(user_id=user_id)
...
def get_connected_mt5_client(
    *,
    ...
    creds = get_mt5_credentials(user_id=user_id, mt5_login=mt5_login)
    if not creds:
    ...
    if not client.connect(
        ...
        client.shutdown()
        raise RuntimeError("Failed to connect to MT5. Please ensure MT5 terminal is running.")
    return client
