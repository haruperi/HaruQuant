"""Minimal SQLite-first migration runner for the agentic backend."""
...
def discover_migrations(migrations_dir: str | Path | None = None) -> list[MigrationRecord]:
    ...
    root = Path(migrations_dir) if migrations_dir is not None else _default_migrations_dir()
    if not root.exists():
        return []
    ...
    for path in sorted(root.glob("*.sql")):
        ...
        if not version or not remainder:
            raise InfrastructureError(
                ...
                "Migration filenames must follow '<version>_<name>.sql'.",
        ...
        for migration in discovered:
            ...
            try:
                ...
                raise InfrastructureError(
                    ...
                    details={"version": migration.version, "path": str(migration.path)},
                ) from exc
