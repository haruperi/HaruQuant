"""Multitasking utilities for concurrent execution.

Provides decorators and functions for running tasks in parallel using
threads or processes with configurable pools and semaphores.
"""
...
def set_max_threads(threads: Optional[int] = None) -> None:
    ...
    """
    previous_threads = config["MAX_THREADS"]
    ...
    logger.info(
        f"Set max threads to {config['MAX_THREADS']} "
        f"(previously {previous_threads})"
    )
...
def set_engine(kind: str = "") -> None:
    ...
    logger.info(
        f"Execution engine set to {config['ENGINE']} " f"(previously {previous_engine})"
    )
...
def task(callee: Callable[..., Any]) -> Callable[..., Optional[Union[Thread, Process]]]:
    ...
    def _run_via_pool(*args: Any, **kwargs: Any) -> Any:
        """Handle semaphore-controlled execution internally.
    ...
    def async_method(*args: Any, **kwargs: Any) -> Optional[Union[Thread, Process]]:
        """Execute the decorated function with task behavior.
