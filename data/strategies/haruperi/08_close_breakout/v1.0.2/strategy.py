...
class CloseBreakoutStrategy(BaseStrategy):
    ...
    def on_bar(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        long_cond = data["close"] > data["prev_close"]
        
        # Short: Close < Prev Close
        short_cond = data["close"] < data["prev_close"]
        
        data.loc[long_cond, "entry_signal"] = 1
        data.loc[short_cond, "entry_signal"] = -1
        
        # Price: We want to execute at Close. 
        # EventDrivenEngine defaults to Close if price is not set, 
        # but to be explicit we can set it.
        # However, if we set it, we ensure clarity.
        data.loc[long_cond, "price"] = data.loc[long_cond, "close"]
        data.loc[short_cond, "price"] = data.loc[short_cond, "close"]
