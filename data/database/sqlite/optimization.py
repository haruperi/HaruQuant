...
class OptimizationManager:
    ...
    def create_optimization_run(
        ...
        try:
            ...
            unsupervised_config_json = (
                json.dumps(unsupervised_config) if unsupervised_config else None
            )
            ...
            INSERT INTO optimization_runs (
                ...
                parameter_space, objective_function, constraints, unsupervised_config,
    ...
    def update_optimization_status(
        ...
        unsupervised_context: Optional[Dict[str, Any]] = None,
        ...
        try:
            ...
            if unsupervised_status is not None:
                update_parts.append("unsupervised_status = ?")
                params.append(unsupervised_status)

            if unsupervised_report is not None:
                update_parts.append("unsupervised_report = ?")
                params.append(json.dumps(unsupervised_report))

            if unsupervised_context is not None:
                update_parts.append("unsupervised_context = ?")
                params.append(json.dumps(unsupervised_context))
    ...
    def save_optimization_results(
        ...
        try:
            ...
            for res in results:
                data_tuples.append(
                    (
                        ...
                        json.dumps(res.get("unsupervised_report", {}))
                        if res.get("unsupervised_report") is not None
                        else None,
                    )
                )
            ...
            INSERT INTO optimization_results (
                ...
                is_best, is_top_10, overfitting_score, stability_score, unsupervised_report
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """
    ...
    def get_optimization_results(
        ...
        try:
            ...
            for row in rows:
                res = dict(row)
                for field in ["parameters", "unsupervised_report"]:
                    if res.get(field):
    ...
    def save_optimization_summary(
        ...
        try:
            ...
            optimization_id = self.create_optimization_run(
                ...
                unsupervised_config=unsupervised_config,
    ...
    def get_optimization_unsupervised_report(
        self,
        optimization_id: int,
    ) -> Optional[Dict[str, Any]]:
        """Return the persisted unsupervised report for one optimization run."""
        run = self.get_optimization_run(optimization_id)
        if not run:
            return None
        report = run.get("unsupervised_report")
        if report is None:
            return None
        return {
            "optimization_id": optimization_id,
            "status": run.get("unsupervised_status"),
            "config": run.get("unsupervised_config"),
            "report": report,
            "context": run.get("unsupervised_context"),
        }
