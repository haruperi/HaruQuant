...
class SchemaManager:
    ...
    def initialize_database(self) -> bool:
        ...
        try:
            ...
            if "governance_strategy_id" not in strategy_columns:
                cursor.execute("ALTER TABLE strategies ADD COLUMN governance_strategy_id TEXT")
            if "artifact_root" not in strategy_columns:
                cursor.execute("ALTER TABLE strategies ADD COLUMN artifact_root TEXT")
            if "strategy_family" not in strategy_columns:
                cursor.execute("ALTER TABLE strategies ADD COLUMN strategy_family TEXT")
            cursor.execute(
            ...
            # =========================================================================
            # BACKTEST SNAPSHOT: Canonical JSON-backed backtest store
            # =========================================================================
            ...
            """
            cursor.execute(create_backtests_table)

            cursor.execute("DROP TABLE IF EXISTS backtest_trade_events")
            cursor.execute("DROP TABLE IF EXISTS backtest_trades")
            cursor.execute("DROP TABLE IF EXISTS backtest_equity_curve")
            cursor.execute("DROP TABLE IF EXISTS backtest_runs")

            cursor.execute("DROP TABLE IF EXISTS finance_trade_metrics")
            cursor.execute("DROP TABLE IF EXISTS finance_return_metrics")
            cursor.execute("DROP TABLE IF EXISTS finance_drawdown_metrics")
            cursor.execute("DROP TABLE IF EXISTS finance_ratio_metrics")
            cursor.execute("DROP TABLE IF EXISTS finance_risk_metrics")
            cursor.execute("DROP TABLE IF EXISTS finance_efficiency_metrics")
            cursor.execute("DROP TABLE IF EXISTS finance_benchmark_metrics")
            cursor.execute("DROP TABLE IF EXISTS finance_distributions")
            ...
            CREATE TABLE IF NOT EXISTS optimization_runs (
                ...
                constraints TEXT,
                unsupervised_config TEXT,
                unsupervised_status TEXT,
                unsupervised_report TEXT,
                unsupervised_context TEXT,
            ...
            optimization_run_columns = {row[1] for row in cursor.fetchall()}
            if "unsupervised_config" not in optimization_run_columns:
                cursor.execute("ALTER TABLE optimization_runs ADD COLUMN unsupervised_config TEXT")
            if "unsupervised_status" not in optimization_run_columns:
                cursor.execute("ALTER TABLE optimization_runs ADD COLUMN unsupervised_status TEXT")
            if "unsupervised_report" not in optimization_run_columns:
                cursor.execute("ALTER TABLE optimization_runs ADD COLUMN unsupervised_report TEXT")
            if "unsupervised_context" not in optimization_run_columns:
                cursor.execute("ALTER TABLE optimization_runs ADD COLUMN unsupervised_context TEXT")
            ...
            optimization_result_columns = {row[1] for row in cursor.fetchall()}
            if "unsupervised_report" not in optimization_result_columns:
                cursor.execute("ALTER TABLE optimization_results ADD COLUMN unsupervised_report TEXT")
            ...
            CREATE TABLE IF NOT EXISTS walk_forward_windows (
                ...
                FOREIGN KEY (optimization_id) REFERENCES optimization_runs (optimization_id) ON DELETE CASCADE,
                FOREIGN KEY (train_backtest_id) REFERENCES backtests (id) ON DELETE SET NULL,
                FOREIGN KEY (test_backtest_id) REFERENCES backtests (id) ON DELETE SET NULL
            );
            """
            ...
            try:
                cursor.execute(
                    "ALTER TABLE edge_profile_snapshots ADD COLUMN unsupervised_summary TEXT"
                )
            except Exception:
                pass
            ...
            CREATE TABLE IF NOT EXISTS risk_runs (
                ...
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (backtest_id) REFERENCES backtests (id) ON DELETE SET NULL
            );
            """
            ...
            conn.commit()
            logger.info(
                "Database schema initialized successfully with JSON backtests + optimization + live trading + edge discovery + risk storage"
            )
            logger.info("  Backtests: backtests")
