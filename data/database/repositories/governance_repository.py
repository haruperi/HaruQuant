...
class GovernanceRepository:
    ...
    def list_votes(self, approval_id: str) -> list[ApprovalVoteRecord]:
        with self._connect() as connection:
            rows = connection.execute(
                """
                SELECT *
        ...
        return [ApprovalVoteRecord(**dict(row)) for row in rows]
    ...
    def update_approval_state(
        ...
        *,
        ...
        if updated is None:
            raise LookupError(f"approval not found after update: {approval_id}")
        return updated
    ...
    def upsert_strategy(
        ...
        *,
        ...
        record = self.get_strategy(strategy_id)
        if record is None:
            raise LookupError(f"strategy not found after upsert: {strategy_id}")
        return record
