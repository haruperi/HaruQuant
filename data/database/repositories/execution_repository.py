...
class ExecutionRepository:
    ...
    def update_intent_status(
        ...
        *,
        ...
        current = self.get_intent(execution_intent_id)
        if current is None:
            raise LookupError(f"execution intent not found: {execution_intent_id}")
        with self._connect() as connection:
            connection.execute(
                """
                UPDATE core_execution_intents
                SET status = ?,
                    pre_send_validation_snapshot_ref = ?,
                ...
                (
                    status,
                    pre_send_validation_snapshot_ref if pre_send_validation_snapshot_ref is not None else current.pre_send_validation_snapshot_ref,
                    execution_intent_id,
                ),
            )
        updated = self.get_intent(execution_intent_id)
        if updated is None:
            raise LookupError(f"execution intent not found after update: {execution_intent_id}")
        return updated
